...
labevents_tble <- tbl(con_bq, "labevents") |>
  ...
  slice_max(storetime, by = c(subject_id, stay_id, label), n = 1) |>
...
chartevents_tble <- tbl(con_bq, "chartevents") |>
  ...
  select(subject_id, stay_id, label, valuenum, charttime, intime) |>
  filter(charttime >= intime) |>
  slice_min(charttime, by = c(subject_id, stay_id, label), n = 1) |>
  pivot_wider(names_from = label,
              values_from = valuenum) |>
...
mimic_icu_cohort_processed <- mimic_icu_cohort |>
  select(-subject_id, -hadm_id, -stay_id, -intime, 
         -outtime, -admittime, -dischtime, -deathtime, 
         -admit_provider_id, -edregtime, -edouttime, -anchor_age, 
         -anchor_year, -anchor_year_group) |>
  print(width = Inf)

tbl_summary(mimic_icu_cohort_processed, by = los_long)
