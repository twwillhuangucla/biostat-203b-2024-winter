...
labevents_tble <- arrow::open_dataset("./labevents.parquet/part-0.parquet", format = "parquet") |>
  ...
  collect() |>
  mutate(stay_id = icustays_tble$stay_id[match(subject_id, icustays_tble$subject_id)]) |>
  mutate(intime = icustays_tble$intime[match(stay_id, icustays_tble$stay_id)]) |>
  mutate(value = as.numeric(value)) |>
  mutate(itemname = d_labitems$label[match(itemid, d_labitems$itemid)]) |>
  group_by(stay_id) |>
  filter(storetime == max(storetime < intime)) |>
  ungroup() |>
  select(subject_id, stay_id, itemname, value) |>
