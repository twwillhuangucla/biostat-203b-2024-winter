...
```{r, eval = FALSE}
labevents_tidyverse <- read_csv("~/mimic/hosp/labevents.csv.gz", col_select = c(subject_id, itemid, charttime, valuenum))
```
...
```{bash, eval = FALSE}
(zcat < ~/mimic/hosp/labevents.csv.gz | awk -F, 'BEGIN {OFS=","} ; NR==1 {print $2,$5,$7,$10} ; NR>1 {if ($5 == 50912 || $5 == 50971 || $5 == 50983 || $5 == 50902 || $5 == 50882 || $5 == 51221 || $5 == 51301 || $5 == 50931) print $2,$5,$7,$10}' | gzip) > labevents_filtered.csv.gz
```
...
```
**Answer:** The first 10 lines of the new file `labevents_filtered.csv.gz` are displayed. There are 24855910 lines in this new file. The `read_csv` function takes 16.046s to ingest `labevents_filtered.csv.gz`. 
...
dplyr::arrange(labevents_arrow, subject_id) %>% head(10)
...
```{r, eval = FALSE}
arrow::write_dataset(arrow::open_dataset("labevents.csv", format = "csv"), ".", format = "parquet")
```
```{r}
system.time(
  labevents_parquet <- arrow::open_dataset("part-0.parquet", format = "parquet") %>% 
    dplyr::select(subject_id, itemid, charttime, valuenum) %>% 
    dplyr::filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) %>% 
    collect())
dplyr::arrange(labevents_parquet, subject_id) %>% head(10)
nrow(labevents_parquet)
```
**Answer:** The Parquet file is 1.9GB which is significantly smaller than the original labevents.csv file. The ingest+select+filter process of the Parquet file took 7.3s which is also a lot faster. The number of rows and first 10 rows of the result tibble match those in Q2.3. \
...
```{r}
system.time(
  labevents_duckdb <- arrow::open_dataset("part-0.parquet", format = "parquet") %>% 
    arrow::to_duckdb() %>%
    dplyr::select(subject_id, itemid, charttime, valuenum) %>% 
    dplyr::filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) %>%
    collect())
dplyr::arrange(labevents_duckdb, subject_id) %>% head(10)
nrow(labevents_duckdb)
```
**Answer:** The ingest+convert+select+filter process took 10.1s. The number of rows and first 10 rows of the result tibble match those in Q2.3. \
