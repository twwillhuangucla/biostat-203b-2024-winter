...
system.time(
  labevents_arrow <- arrow::open_dataset("labevents.csv", format = "csv") %>%
    ...
    mutate(charttime = force_tz(charttime, tzone = "US/Eastern")) %>%
    collect()
)
...
system.time(
  labevents_parquet <- 
    arrow::open_dataset("./labevents.parquet/part-0.parquet", format = "parquet") %>%
    dplyr::select(subject_id, itemid, charttime, valuenum) %>%
    dplyr::filter(itemid %in% 
      c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) %>%
    collect()
)
...
labevents_duckdb$charttime <- 
  with_tz(labevents_duckdb$charttime, tzone = "America/New_York")
...
dplyr::arrange(chartevents_parquet, subject_id) %>% head(10)
```
